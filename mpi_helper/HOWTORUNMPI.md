...
A JDK25 is required with support for [JEP 454](https://openjdk.org/jeps/454).

The actual command will be highly specific to your environment (thanks to MPI) but we will highlight some common requirements using an example on macOS:

```
~/software/openmpi_install/bin/mpirun -np 4 ~/software/jdk25/jdk-25.jdk/Contents/Home/bin/java --add-modules=jdk.incubator.vector --enable-preview --enable-native-access=ALL-UNNAMED -Djava.library.path=~/software/openmpi_install/lib -jar ogolem-snapshot.jar --core ar38_sweden.ogo
```
...
The `foreign` feature must be granted permissions with `--enable-native-access=ALL-UNNAMED` (a reduced set may be possible). Also, the vectorAPI incubator must be enabled as usual.
