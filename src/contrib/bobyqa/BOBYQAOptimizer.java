...
public class BOBYQAOptimizer implements Serializable {
    ...
    /**
     * @param boundaries
     *            Boundaries for objective variables. Will not be used if except
     *            for their dimensions if the methodDoesNormalize flag is set to true
     ...
     * @param methodDoesNormalize
     *            if the BOBYQAMethod used does normalize the parameters before entry. If so, the boundaries argument will be ignored and [0,1] for all parameters assumed
     */
    public BOBYQAOptimizer(final double[][] boundaries,
            final int numberOfInterpolationPoints, final double initialTrustRegionRadius,
            final double stoppingTrustRegionRadius, final int maxIterations,
            final boolean methodDoesNormalize) {
        
        if(methodDoesNormalize){
            // explictly fix the bounds            
            this.boundaries = new double[2][boundaries[0].length];
            for(int i = 0; i < boundaries[0].length; i++)
                this.boundaries[1][i] = 1.0;
        } else{
    ...
    public BOBYQAOptimizer(BOBYQAOptimizer orig){
        this.boundaries = orig.boundaries.clone();
        this.dimension = orig.dimension;
        this.initialTrustRegionRadius = orig.initialTrustRegionRadius;
        this.isMinimize = orig.isMinimize;
    ...
    public Tuple<Double,double[]> doOptimize(final double[] guess, final BOBYQAMethod met) {
        // -------------------- Initialization --------------------------------
        isMinimize = true;
        // number of objective variables/problem dimension
        dimension = guess.length;
        try{
            checkParameters(guess);
        } catch (Exception e){
            e.printStackTrace(System.err);
            return new Tuple<>(org.ogolem.core.FixedValues.NONCONVERGEDENERGY,guess);
        }
        ...
        double value;
        try{
            value = bobyqa(dimension, numberOfInterpolationPoints, x, xl,
                xu, initialTrustRegionRadius, stoppingTrustRegionRadius,
                maxIter, met);
        } catch(Exception e){
            //XXX better this. exception sucks in this context.
            e.printStackTrace(System.err);
            return new Tuple<>(org.ogolem.core.FixedValues.NONCONVERGEDENERGY,guess);
        }

        
        return new Tuple<>(value, x.getAll());
    }
    ...
    private double bobyqa(
        ...
        if (npt < n + 2 || npt > (n + 2) * np / 2)
            throw new Exception("NPT_IS_NOT_IN_THE_REQUIRED_INTERVAL " + npt + " " + n + " " + np);
        ...
        for (j = 1; j <= i__1; j++) {
            ...
            if (temp < rhobeg + rhobeg) {
                throw new Exception("ONE_OF_THE_BOUND_DIFFERENCES_TOO_LOW" + temp);
            }
            jsl = isl + j - 1;
            jsu = jsl + n;
        ...
        return bobyqb(n, npt, x, xl, xu, rhobeg, rhoend, maxfun,
                ...
                w.ptr(id_-1), w.ptr(ivl-1), w.ptr(iw-1), met);
    } // bobyqa
    ...
    private double bobyqb(
        ...
        fsave = fval.get(1);
        if (nf.value < npt) { // should not happen (JMD:I do not care. better be save than sorry.)
            return met.computeObjectiveValue(x.getAll());
        ...
        case 190: {
            ...
            kbase = kopt.value;
                    ...
                    kopt, vlag, w.ptr(-2), w.ptr(n+np-1), w.ptr(ndim+np-1), met);
        ...
        case 230: {
            ...
            if (ntrits == 0) {
                ...
                if (denom <= half * (d__1 * d__1)) {
                    ...
                    throw new Exception("TOO_MUCH_CANCELLATION " + vquad);
                }
        ...
        case 360: {
            ...
            if (nf.value > maxfun) { // should not happen,
                // TooManyEvaluationsException is thrown before // JMD this is an original comment and does not seem to apply to our usage...
                // JMD throw new RuntimeException("Return from BOBYQA because the objective function has been called max_f_evals times.");
                System.out.println("Return from BOBYQA because the objective function has been called max_f_evals times.");
                // JMD last evaluation, just to be sure
                return met.computeObjectiveValue(x.getAll());
            }
            nf.value++;
            f = met.computeObjectiveValue(x.getAll());
            if (!isMinimize)
                f = -f;
            if (ntrits == -1) {
                fsave = f;
            ...
            if (ntrits > 0) {
                if (vquad >= zero) {
                    throw new Exception("TRUST_REGION_STEP_FAILED " + vquad);
                }
                ratio = (f - fopt) / vquad;
                if (ratio <= tenth) {
                    // Computing MIN
    ...
    private void rescue (
        ...
        case 260: {
            ...
            for (kpt = 1; kpt <= i__1; kpt++) {
                ...
                nf.value++;
                f = met.computeObjectiveValue(w.getAll(1,n));
                if (!isMinimize)
                    f = -f;
                fval.set(kpt, f);
     ...
     */
    private void checkParameters(double[] init) throws Exception{

        if (boundaries != null) {
            if (boundaries.length != 2) {
                throw new Exception( "Boundaries dims wrong! " +
                        boundaries.length + " " +
                        2);
            }
            ...
            if (boundaries[0].length != init.length) {
                throw new Exception( "Length of lower boundaries too long or short! " +
                        boundaries[0].length + " " +
                        init.length);
            }
            if (boundaries[1].length != init.length) {
                throw new Exception( "Length of upper boundaries too long or short! " +
                        boundaries[1].length + " " +
                        init.length);
            }
            for (int i = 0; i < init.length; i++) {
                if (boundaries[0][i] > init[i]) {
                    System.err.println("WARNING: Initial values out of boundaries (I)! " + init[i] + " " + boundaries[0][i] + " " +
                            boundaries[1][i] + " At position: " + i + ". Adjusting...");
                    init[i] = boundaries[0][i];
                } else if(boundaries[1][i] < init[i]){
                    System.err.println("WARNING: Initial values out of boundaries (II)! " + init[i] + " " + boundaries[0][i] + " " +
                            boundaries[1][i] + " At position: " + i + ". Adjusting...");
