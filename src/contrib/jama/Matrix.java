 ...
 * ------------------------ */
   ...
   */
   ...
   public static Matrix identity (final int m, final int n) {
      ...
      for (int i = 0; i < m; i++) {
         // by Java standard, the arrays must be initialized to zero. hence,
         // a new matrix is always zero
         /*for (int j = 0; j < n; j++) {
            X[i][j] = (i == j ? 1.0 : 0.0);
         }*/
         X[i][i] = 1.0;
   ...
   public static Matrix read (BufferedReader input) throws java.io.IOException {
	...
	throw new java.io.IOException("Unexpected EOF on matrix read.");
      ...
      for (int j=0; j<n; j++)  // extract the elements of the 1st row.
         row[j]=(v.get(j)).doubleValue();
      v.clear();
      final List<double[]> v2 = new ArrayList<>();
      v2.add(row);  // Start storing rows instead of columns.
      while (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
         // While non-empty lines
         v2.add(row = new double[n]);
         int j = 0;
         do {
            if (j >= n) throw new java.io.IOException
               ("Row " + v2.size() + " is too long.");
            row[j++] = Double.valueOf(tokenizer.sval).doubleValue();
         } while (tokenizer.nextToken() == StreamTokenizer.TT_WORD);
         if (j < n) throw new java.io.IOException
            ("Row " + v2.size() + " is too short.");
      }
      int m = v2.size();  // Now we've got the number of rows.
      double[][] A = new double[m][];
      for(int x = 0; x < m; x++){
          A[x] = v2.get(x).clone(); // copy the rows out of the vector
      }
      return new Matrix(A);
   }
